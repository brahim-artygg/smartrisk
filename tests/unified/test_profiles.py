...
def test_profiles_have_expected_limits():
    ...
    assert FREE_PROFILE.name == "quick_free_v1"
    assert FREE_PROFILE.max_holder_contract_probes == 0
    assert FREE_PROFILE.max_rpc_requests == 16
    assert FREE_PROFILE.max_log_chunks == 2
