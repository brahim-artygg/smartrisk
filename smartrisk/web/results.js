...
function renderPublicSummary(report) {
  ...
  const measureRows = [
    ...
    ['Upgradeable proxy', yn(m('contract.proxy_detected')), 'Bytecode / EIP-1967'],
	  ...
	  const ai = report?.ai_explanation;
	  const aiHtml = ai?.status === 'complete' && ai.summary ? `<div class="data-row"><div class="data-main"><div class="data-title">AI explanation</div><div class="data-sub">${esc(ai.summary)} · This explanation does not change the score or verdict.</div></div><div class="data-value">${esc(ai.model || 'AI')}</div></div>` : '';
	  els.technicalContent.innerHTML = `<div class="rows">${aiHtml}${checkHtml}${measureHtml}${engineRows}${dimensions.map(item => `<div class="data-row"><div class="data-main"><div class="data-title">${esc(item.label || item.id || 'Risk dimension')}</div><div class="data-sub">${esc(item.severity || 'unknown')} · ${esc(item.signals == null ? 'No signal count' : `${item.signals} signal(s)`)}</div></div><div class="data-value">Public</div></div>`).join('')}${unknownRows}${budgetHtml}<div class="data-row"><div class="data-main"><div class="data-title">Full report</div><div class="data-sub">Available through an eligible paid API key.</div></div><div class="data-value">Locked</div></div></div>`;
}
